...
type INotice interface {
	TransferNFTS(ctx context.Context, params *vo.TransferNFTS) (*dto.TransferNFTS, error)
	TransferClasses(ctx context.Context, params *vo.TransferClasses) (*dto.TransferClasses, error)
}
...
func (a *notice) TransferNFTS(ctx context.Context, params *vo.TransferNFTS) (*dto.TransferNFTS, error) {
	...
	grpcClient, ok := initialize.NativeNoticeClientMap[mapKey]
	if !ok {
...
func (a *notice) TransferClasses(ctx context.Context, params *vo.TransferClasses) (*dto.TransferClasses, error) {
	logger := a.logger.WithField("params", params).WithField("func", "transfer class")
	path := ctx.Value(httptransport.ContextKeyRequestPath).(string)[len(configs.Cfg.App.RouterPrefix)+1:]
	res := &dto.TransferClasses{}
	project, err := a.getProject(params.ProjectID)
	if err != nil {
		logger.WithError(err).Error("query project")
		return res, err
